-- Kolonne til den nye Ungtyr-kategori på bookings (se
-- 20260917150000_add_ungtyr_category.sql), og opdatering af
-- validate_booking() så animal_count og statusnulstillingen ved
-- transportør-rettelse af en godkendt booking også dækker den. Se
-- CLAUDE.md.

alter table public.bookings
  add column n_ungtyr integer not null default 0;

alter table public.bookings
  add constraint bookings_n_ungtyr_check check (n_ungtyr >= 0);
